...
def _as_source_list(data: DataInput) -> list[DataSource]:
    ...
    Args:
        data: A single path/DataFrame, or a sequence of them.
...
def audit(
    ...
    Args:
        train: Training data: a single .csv/.parquet path or DataFrame, or a
            list mixing either.
        test: Test data, in the same shapes as ``train``.
        smiles_column: Name of the SMILES column, resolved per source (exact
            match, then case-insensitive, then a common alias like ``smi``
            or ``structure``).
        identifier_columns: Optional compound identifier column names to
            cross-check for leakage/namespace issues.
        tautomer_standardisation: Whether to canonicalise tautomers before
            comparing molecules.
        max_train_test_similarity: Tanimoto similarity threshold (0.0-1.0,
            inclusive) at or above which a train/test pair is flagged as
            possible leakage.
        fp_radius: Morgan fingerprint radius.
        fp_n_bits: Morgan fingerprint bit-vector length.
        report_output: Optional path to also write the full report to. If
            omitted, the report is only returned, not written to disk.
        report_format: ``"json"`` or ``"txt"``. If omitted, it's inferred
            from ``report_output``'s extension (``.txt`` -> ``"txt"``,
            otherwise ``"json"``); has no effect if ``report_output`` is
            omitted.
        print_report: Whether to print the colour-coded summary dashboard to
            stdout.
