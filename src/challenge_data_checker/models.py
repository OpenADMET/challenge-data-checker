...
class MoleculeRecord:
    ...
    Attributes:
        ...
        identifiers: Mapping of identifier column name to its value for this
            row (``None`` if the column was absent or the cell was empty).
        parse_error: Description of why parsing/processing failed, or
            ``None`` if the row was processed successfully.
        canonical_smiles: RDKit canonical SMILES, or ``None`` if unparsed.
        inchikey: InChIKey, or ``None`` if unparsed or InChIKey generation
            failed.
        fingerprint: Stereo-blind Morgan fingerprint, or ``None`` if unparsed.
        heavy_atom_count: Number of heavy atoms, or ``None`` if unparsed.
        num_fragments: Number of disconnected fragments, or ``None`` if
            unparsed.
        has_dot: Whether the raw SMILES contains a ``.`` fragment separator.
        contains_metal: Whether any atom is a transition/heavy/alkali(-earth)
            metal.
        is_mixture: Whether the molecule has multiple disconnected fragments.
        is_salt_or_metal: Whether the molecule is a salt or metal complex.
        is_suspicious_fragment: Whether the molecule looks like a stripped
            degradation artefact (e.g. a bare counterion).
